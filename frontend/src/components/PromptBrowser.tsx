...
export default function PromptBrowser({ open, onClose, onChanged }: PromptBrowserProps) {
  ...
  useEffect(() => {
    if (!open) return;
    listPersona().then((data) => setFiles(data.files));
  }, [open]);
  ...
  const grouped = files.reduce<Record<string, PersonaFileInfo[]>>((acc, f) => {
  ...
  return (
    <Overlay open={open} onClose={onClose} title={selected ? selected : "Persona Prompts"}>
      {selected ? (
